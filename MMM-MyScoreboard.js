...
Module.register("MMM-MyScoreboard", {
  ...
  boxScoreFactory: function (league, gameObj) {
    ...
    if (this.viewStyleHasLogos(viewStyle)) {
      ...
      if (
        this.localLogos[leagueForLogoPath] &&
        this.localLogos[leagueForLogoPath].indexOf(gameObj.hTeam + ".svg") !==
          -1
      ) {
        hTeamLogoImg.src = this.file(
          "logos/" + leagueForLogoPath + "/" + gameObj.hTeam + ".svg"
        );
      } else if (
        this.localLogos[leagueForLogoPath] &&
        this.localLogos[leagueForLogoPath].indexOf(gameObj.hTeam + ".png") !==
          -1
      ) {
        hTeamLogoImg.src = this.file(
          "logos/" + leagueForLogoPath + "/" + gameObj.hTeam + ".png"
        );
      } else {
      ...
      if (
        this.localLogos[leagueForLogoPath] &&
        this.localLogos[leagueForLogoPath].indexOf(gameObj.vTeam + ".svg") !==
          -1
      ) {
        vTeamLogoImg.src = this.file(
          "logos/" + leagueForLogoPath + "/" + gameObj.vTeam + ".svg"
        );
      } else if (
        this.localLogos[leagueForLogoPath] &&
        this.localLogos[leagueForLogoPath].indexOf(gameObj.vTeam + ".png") !==
          -1
      ) {
        vTeamLogoImg.src = this.file(
          "logos/" + leagueForLogoPath + "/" + gameObj.vTeam + ".png"
        );
      } else {
  ...
  socketNotificationReceived: function (notification, payload) {
    if (
      ...
      notification === "MMM-MYSCOREBOARD-LOCAL-LOGO-LIST" &&
      payload.instanceId == this.identifier
    ) {
  ...
  start: function () {
    ...
    */
    ...
    self.sendSocketNotification("MMM-MYSCOREBOARD-GET-LOCAL-LOGOS", {
